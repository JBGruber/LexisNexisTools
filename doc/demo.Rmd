...
The function `lnt_similarity()` combines the fast similarity measure from [**quanteda.textstats**](https://github.com/quanteda/quanteda.textstats) with the much slower but more accurate relative [Levenshtein distance](https://en.wikipedia.org/wiki/Levenshtein_distance) to compare all articles published on the same day.
