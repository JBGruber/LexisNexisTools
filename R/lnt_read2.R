...
lnt_read2 <- function(x,
  ...
  df.l <- lapply(articles.l, function(a) {
    ...
    if (!is.na(len)) {
      ...
      # make data.frame
      ### length
      . <- grep(pattern = length_keyword, x = meta, value = TRUE)[1]
      length.v <- gsub(length_keyword, "", .)
      
      ### Newspaper (first non-emtpy line)
      newspaper.v <- grep(pattern = "^$",
                          x = meta, 
                          value = TRUE,
                          fixed = FALSE, 
                          invert = TRUE)[1]
      ...
      ### Date
      . <- stringi::stri_extract_last_regex(str = meta[seq_len(5)],
                                            pattern = "ö\\w+ \\d+, \\d+|\\d+ \\w+ \\d+")
      date.v <- .[!is.na(.)][1]
      
      
      ### Author (where available)
      . <- grep(pattern = "AUTOR: |VON |BYLINE: ", x = meta, value = TRUE)[1]
      author.v <- gsub("AUTOR: |VON |BYLINE: ", "", .)
      
      ### section (where available)
      . <- grep(pattern = "SECTION: |RUBRIK: ", x = meta, value = TRUE)[1]
      section.v <- gsub("SECTION: |RUBRIK: ", "", .)
      
      ### edition (where available)
      date <- grep(date.v, x = meta, fixed = TRUE)
      if (length(date) == 1) {
        d1 <- meta[(date + 1):(date + 2)]
        if (!d1[1] == "") {
          edition.v <- d1[1]
          ...
          edition.v <- paste(edition.v, 
                             grep("edition", article[!stringi::stri_isempty(str = article)][1], 
                                  value = TRUE,
                                  ignore.case = TRUE),
                             collapse = "; ")
        }
      ...
      ### Headline
      headline.v <- meta
      headline.v[c(grep(length.v, headline.v, fixed = TRUE),
                   grep(date.v, headline.v, fixed = TRUE),
                   grep(newspaper.v, headline.v, fixed = TRUE),
                   grep(author.v, headline.v, fixed = TRUE),
                   grep(section.v, headline.v, fixed = TRUE))] <- ""
      
      headline.v <- paste(headline.v, collapse=" ")
      
      meta.df <- data.frame(ID = 1,
      ...
      art.v <- article[1:(grep(end_keyword, article) - 1)]
      
      articles.df <- data.frame(ID = 1,
                                Article = art.v,
                                stringsAsFactors = FALSE)
      if(extract_paragraphs){
        art.v[length(art.v) + 1] <- ""
        empties <- grep("^$", art.v)
        empties <- empties[-which(diff(empties) == 1)]
        paragraphs.v <- unlist(sapply(seq_len(length(empties)), function(e) {
          if (!e == length(empties)) {
            paste(art.v[empties[e]:empties[e + 1]], 
                  collapse=" ")
          } else {
            NULL
          }
        ...
        paragraphs.df <- data.frame(Art_ID = 1,
                                    Par_ID = seq_len(length(paragraphs.v)),
                                    Paragraph = paragraphs.v,
                                    stringsAsFactors = FALSE)
      }else{
      ...
      list(meta.df = meta.df,
           articles.df = articles.df,
           paragraphs.df = paragraphs.df)
    } else {
      NULL
    }
  })
  ...
  # correct IDs
  for(i in seq_len(length(df.l) - 1)) {
    df.l[[1 + 1]]$meta.df$ID <- df.l[[1 + 1]]$meta.df$ID + max(df.l[[1]]$meta.df$ID)
    df.l[[1 + 1]]$articles.df$ID <- df.l[[1 + 1]]$articles.df$ID + max(df.l[[1]]$articles.df$ID)
    df.l[[1 + 1]]$paragraphs.df$Art_ID <- df.l[[1 + 1]]$paragraphs.df$Art_ID + max(df.l[[1]]$paragraphs.df$Art_ID)
    df.l[[1 + 1]]$paragraphs.df$Par_ID <- df.l[[1 + 1]]$paragraphs.df$Par_ID + max(df.l[[1]]$paragraphs.df$Par_ID)
  }
  
  # Remove leading and trailing whitespace
  meta.df <- data.table::rbindlist(lapply(df.l, function(i) i$meta.df))
  for (col in colnames(meta.df)) {
    meta.df <- meta.df[,  col := gsub("^\\s+|\\s+$", "", as.character(col)), by = ID]
  }
  articles.df <- data.table::rbindlist(lapply(df.l, function(i) i$articles.df))
  articles.df <- articles.df[,  Article := gsub("^\\s+|\\s+$", "", as.character(Article)), by = ID]
  
  paragraphs.df <- data.table::rbindlist(lapply(df.l, function(i) i$paragraphs.df))
  paragraphs.df <- paragraphs.df[,  Paragraph := gsub("^\\s+|\\s+$", "", as.character(Paragraph)), by = Par_ID]
