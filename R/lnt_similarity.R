...
#'   duplicate.
#' @param Rel.diff.on Calculate the relative Levenstein distance between two
...
lnt_similarity <- function(texts, dates, IDs = NULL, threshold = 0.99, Rel.diff.on=FALSE) {
  ...
  duplicates.df <- lapply(dates.d ,FUN=function(dates.d){
    ...
    if (nrow(dup)>0){
      ...
      dup[,1] <- sapply(seq_len(nrow(dup)), function(i) dup[i,1] <- rownames(sample_sim)[dup[i,1]])
      dup[,2] <- sapply(seq_len(nrow(dup)), function(i) dup[i,2] <- rownames(sample_sim)[dup[i,2]])
      ...
      if(Rel.diff.on){
        duplicates.df$Rel.diff. <- sapply(seq_len(nrow(dup)), function(i) adist(texts[match(dup[i,2],IDs)],texts[match(dup[i,1],IDs)], ignore.case = TRUE)/# string distance
