...
lnt_read3 <- function(x,
  ...
  art.v <- sapply(df.l, function(i) {
    end <- grep(end_keyword, i$article)
    if (length(end) > 0) {
      trimws(paste(i$article[1:end], collapse=" "), which = "both")
    } else {
      trimws(paste(i$article, collapse=" "), which = "both")
    }
  })
  ...
  paragraphs <- lapply(df.l, function(i) {
    art.v <- i$article
    art.v[length(art.v) + 1] <- ""
    empties <- grep("^$", art.v)
    empties <- empties[-which(diff(empties) == 1)]
    paragraphs.v <- sapply(seq_len(length(empties)), function(e) {
      if (!e == length(empties)) {
    ...
    trimws(paragraphs.v, which = "both")
  })
